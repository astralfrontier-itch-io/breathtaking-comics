\startenvironment env_book

% Assume source files are UTF-8
\enableregime[utf]

% Set up PDF metadata
\setupinteraction[state=start, title=Breathtaking Comics, author=Astral Frontier]
\setupbackend[format=PDF/X-1a:2003, intent=SWOP2006_Coated5v2.icc]

% Page size is US-Letter
\setuppapersize[letter,portrait]

% Page numbers are at the bottom
\setuppagenumbering[]
\setupheadertexts[Breathtaking Comics]
\setupfootertexts[chapter][pagenumber]

% Don't show section numbers
\setupheads[number=no]

% Center-align titles and chapters
\setuphead[title,chapter][align=middle]

% Space between paragraphs
\setupwhitespace[halfline]

% Don't add extra whitespace in itemized lists
\setupitemize[packed]

% Image path configuration
\setupexternalfigures[directory=.,location=local]

\stopenvironment